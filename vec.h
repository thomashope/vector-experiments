...
struct vec2
{
    ...
    explicit vec2(const f32* data) : x(data[0]), y(data[1]) {}
    ...
    static vec2 fromAngle(f32 radians) { return { std::cos(radians), std::sin(radians) }; }
    ...
    vec2 rotated(f32 radians) { f32 s = std::sin(radians), c = std::cos(radians); return { x * c - y * s, x * s + y * c }; }
...
struct vec3
{
    ...
    explicit vec3(const f32* data) : x(data[0]), y(data[1]), z(data[2]) {}
    ...
    vec2 pitchYaw() const { vec3 dir = normalized(); return { std::asin(dir.z), std::atan2(dir.y, dir.x) }; } /// Returns a vec2 where X is the pitch and Y is the yaw.
...
struct vec4
{
    ...
    explicit vec4(const f32* data) : x(data[0]), y(data[1]), z(data[2]), w(data[3]) {}
